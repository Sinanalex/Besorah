 ...
 * Enhanced version with persistent storage, performance optimizations, and advanced features
 * Version 3.0
 */
        ...
        try {
            if (this.useNativeStorage) {
                ...
                Object.keys(localStorage)
                    .filter(k => k.startsWith('bible-planner-'))
...
class AnalyticsManager {
    calculateStreak(completedReadings) {
        ...
        // Simple streak calculation (can be enhanced with actual dates)
        const sortedReadings = Object.entries(completedReadings)
            .filter(([_, completed]) => completed)
            .sort((a, b) => b[0].localeCompare(a[0]));
        
        if (sortedReadings.length > 0) {
            streak = Math.min(sortedReadings.length, 30); // Cap at 30 for demo
        }
...
class EthiopianBiblePlanner {
    ...
    // ===================================
    // DATA INITIALIZATION (Abbreviated - same as original)
    // ===================================
            ...
            this.rootElement.innerHTML = `
                <div style="display: flex; align-items: center; justify-content: center; min-height: 50vh; padding: 2rem;">
                    <div style="text-align: center; max-width: 500px;">
                        ...
                        <p style="color: #64748b; margin-bottom: 2rem; line-height: 1.6;">We're sorry for the inconvenience. Please try refreshing the page.</p>
            ...
            this.rootElement.innerHTML = `
                <div style="display: flex; align-items: center; justify-content: center; min-height: 50vh; padding: 2rem;">
                    <div style="text-align: center; max-width: 500px;">
                        ...
                        <p style="color: #64748b; margin-bottom: 2rem;">We're sorry for the inconvenience. Please try refreshing the page.</p>
                        <button onclick="location.reload()" style="background: linear-gradient(135deg, #205782, #f2842f); color: white; padding: 0.75rem 2rem; border-radius: 50px; border: none; font-weight: 600; cursor: pointer;">
                            Refresh Page
                        </button>
                    </div>
                </div>
